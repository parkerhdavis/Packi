...
export function start(initializationFunction,preprocessingFunction,updateFunction,animationFunction){
	...
	// --- Packi integration: accept config updates via postMessage ---
	window.addEventListener('message', (e) => {
		if (!e.data || e.data.type !== 'packi-pbr-update') return;
		var config = e.data.config || {};
		var oldAndNew = SCENE_CONFIGURATION.updateConfiguration(config, CONSTANTS.updateMode.extendCurrent);
	...
	// Notify parent frame that PBR.ONE is ready
	if (window.parent !== window) {
		window.parent.postMessage({ type: 'packi-pbr-ready' }, '*');
	}
}
